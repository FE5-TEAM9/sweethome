...
import { linkAccount } from "~/api/user";
import { AccountModalProps, InputState } from "~/types";
  ...
  const enrollAccount = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const body = {
      ...
      accountNumber:
        ...
        accountInput.input3,
      phoneNumber:
        accountInput.phone0 + accountInput.phone1 + accountInput.phone2,
      signature: isAgree
    };
    ...
    <>
      ...
      <section className={styles.accountModal}>
        <form>
          ...
          {bankCode ? null : (
            <p className={styles.guide}>은행을 선택해 주세요.</p>
          )}
